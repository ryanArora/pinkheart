...
if (process.argv[2] === "-c") {
  // get the color that the user wants
  let color = process.argv[3];
  console.log(typeof color);
  if (typeof color === "undefined" || !color) {
    color = "";
  }
  // switch case to get the color
  switch (color.toLowerCase()) {
    case "red":
      txts = txts.map((str) => colors.red(str));
      break;
    case "green":
      txts = txts.map((str) => colors.green(str));
      break;
    case "yellow":
      txts = txts.map((str) => colors.yellow(str));
      break;
    case "blue":
      txts = txts.map((str) => colors.blue(str));
      break;
    case "magenta":
      txts = txts.map((str) => colors.magenta(str));
      break;
    case "cyan":
      txts = txts.map((str) => colors.cyan(str));
      break;
    case "white":
      txts = txts.map((str) => colors.white(str));
      break;
    case "brightred":
      txts = txts.map((str) => colors.brightRed(str));
      break;
    case "brightgreen":
      txts = txts.map((str) => colors.brightGreen(str));
      break;
    case "brightyellow":
      txts = txts.map((str) => colors.brightYellow(str));
      break;
    case "brightblue":
      txts = txts.map((str) => colors.brightBlue(str));
      break;
    case "brightmagenta":
      txts = txts.map((str) => colors.brightMagenta(str));
      break;
    case "brightcyan":
      txts = txts.map((str) => colors.brightCyan(str));
      break;
    case "brightwhite":
      txts = txts.map((str) => colors.brightWhite(str));
      break;
    default:
      // log available colors if the user doesn't give a valid color
      console.log(
        "Available colors: red, green, yellow, blue, magenta, cyan, white, brightRed, brightGreen, brightYellow, brightBlue, brightMagenta, brightCyan, brightWhite"
      );
      process.exit(1);
  }
  console.log(txts[0]);
  ...
  // set rainbow
  txts = txts.map((str) => colors.rainbow(str));
} else if (process.argv[2] === "-h") {
  console.log("Usage:".rainbow);
  console.log(
    `  ${colors.rainbow("heart")} ${colors.red("-c")} [color]`.brightWhite
  );
  console.log(
    `  ${colors.rainbow("heart")} ${colors.red("-r")} (rainbow)`.brightWhite
  );
  console.log(
    `  ${colors.rainbow("heart")} ${colors.red("-h")} (help)`.brightWhite
  );
  process.exit(0);
}
